...
exports.homes_for_ukraine = (bot) => {
  ...
  // id_check_app|tls_vfs
  bot.hears(
    "Я використовував додаток ID Check, щоб підтвердити особу заявника|Надав документи, що посвідчують особу заявника, використовуючи додатки TLS або VFS",
    async (ctx) => {
      await ctx.replyWithHTML(
  ...
  bot.hears(
    "У мене немає дійсного українського закордонного паспорта|У мене є паспорт, виданий не Україною, а іншою країною",
    async (ctx) => {
      await ctx.replyWithHTML(
        `<b>Якщо у вас немає дійсного українського закордонного паспорта або прострочений український закордонний паспорт зі штампом про продовження терміну дії</b>
    \nВам потрібно буде записатися на прийом до візового центру (VAC) і з'явитися на нього. Це також стосується дітей-заявників.
